...
# Dueling DQN 모델 정의
class Dueling_Doubling_DQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Dueling_Doubling_DQN, self).__init__()
...
# DQN 학습 함수
def train_dueling_doubling_dqn(env, episodes=500, model_path="best_model.pth"):
    input_dim = 9
    output_dim = 3
    model = Dueling_Doubling_DQN(input_dim, output_dim).to(device)
    target_model = Dueling_Doubling_DQN(input_dim, output_dim).to(device)
